...
export const createWithdrawal = async (req, res) => {
  try {
    ...
    // Load withdrawal lock settings
    const Settings = (await import('../models/Settings.js')).default;
    const lockAmountSetting = await Settings.findOne({ key: 'withdrawLockAmount' });
    const lockDaysSetting = await Settings.findOne({ key: 'withdrawLockDays' });
    
    const lockAmount = lockAmountSetting?.value || 65;
    const lockDays = lockDaysSetting?.value || 90;
